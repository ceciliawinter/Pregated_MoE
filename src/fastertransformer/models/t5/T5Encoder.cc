...
void T5Encoder<T>::initialize()
{
    ...
    else if (attention_type_ == AttentionType::UNFUSED_MHA || attention_type_ == AttentionType::UNFUSED_PADDED_MHA) { //select this branch
        attention_layer_ =
    ...
    else if (activation_type_ == ActivationType::Relu || activation_type_ == ActivationType::ReGLU) { // select this branch
                           ...
                           const T5EncoderWeight<T>* t5_encoder_weights)
{
    ...
    for (uint ite = 0; ite < iteration_num; ite++) {
        ...
        switch (attention_type_) {
            ...
            case AttentionType::UNFUSED_PADDED_MHA: { // select this branch
                invokeBuildEncoderAttentionMask(
                    attention_mask_, sequence_lengths, local_batch_size, request_seq_len, stream_);
        ...
        for (uint i = 0; i < num_layer_; i++) {
            ...
            if (has_adapters() && layer_weight->has_adapters()) { // not select this branch
            ...
            if (use_moe) {
                ...
                // printf("layernorm_type_ = %d\n", layernorm_type_); // 0 pre_layernorm,
                if (layernorm_type_ == LayerNormType::pre_layernorm) { //select this branch
