                                     ...
                                     int*              kernel_occupancy = nullptr)
{
    ...
    using GemmKernel_ = typename cutlass::gemm::kernel::DefaultGemmGrouped<
        ElementType,                                    // 矩阵元素类型
        cutlass::layout::RowMajor,                      // 指定矩阵的存储布局为行优先（Row Major）
        cutlass::ComplexTransform::kNone,               // 指定不进行任何复数变换
        MixedGemmArchTraits::ElementsPerAccessA,        // 指定矩阵A的每次访问元素个数
        CutlassWeightType,                              // 权重矩阵的元素类型
        typename MixedGemmArchTraits::LayoutB,          // 指定矩阵B的存储布局?
        cutlass::ComplexTransform::kNone,               // 指定不进行任何复数变换
        MixedGemmArchTraits::ElementsPerAccessB,        // 指定矩阵B的每次访问元素个数    
        ElementType,                                    // 输出矩阵的元素类型 
        cutlass::layout::RowMajor,                      // 指定输出矩阵的存储布局是行优先
        ElementAccumulator,                             // 累加器的元素类型,用于存储中间结果   
        typename MixedGemmArchTraits::OperatorClass,    // 指定操作类型
        arch,                                           // 指定计算架构
        ThreadblockShape,                               // 指定线程块的形状
        WarpShape,                                      // 指定warp的形状
        typename MixedGemmArchTraits::InstructionShape, // 指定指令形状
        EpilogueOp,                                     // 指定epilogue操作,如激活函数？  
        cutlass::gemm::threadblock::GemmBatchedIdentityThreadblockSwizzle,  // 指定线程块调度策略
        Stages,                                         // 指定流水线阶段数
        cutlass::gemm::kernel::GroupScheduleMode::kDeviceOnly,  // 指定组调度模式为仅设备调度
        typename MixedGemmArchTraits::Operator>::GemmKernel;    // 指定操作类型GEMM核 
    ...
    GemmGrouped gemm;
    // /data/FasterTransformer/3rdparty/cutlass/include/cutlass/gemm/kernel/gemm_grouped.h
    // /data/FasterTransformer/3rdparty/cutlass/include/cutlass/gemm/device/base_grouped.h
                                                         ...
                                                         cudaStream_t      stream)
{
    FT_LOG_DEBUG(__PRETTY_FUNCTION__);
    // T = f, WeightType = f
    // is_weight_only = false, only_simt_configs = true
    static constexpr bool          is_weight_only    = !std::is_same<T, WeightType>::value; 
    static constexpr bool          only_simt_configs = std::is_same<T, float>::value;
    // /data/FasterTransformer/src/fastertransformer/kernels/cutlass_kernels/cutlass_heuristic.cc

    std::vector<CutlassGemmConfig> candidate_configs = get_candidate_configs(sm_, is_weight_only, only_simt_configs);
    // tile_config::CtaShape128x128x8_WarpShape64x64x8, SplitKStyle::NO_SPLIT_K, split_k_factor::1, stages::2
    // tile_config::CtaShape128x128x8_WarpShape64x64x8, SplitKStyle::NO_SPLIT_K, split_k_factor::1, stages::3
    // tile_config::CtaShape128x128x8_WarpShape64x64x8, SplitKStyle::NO_SPLIT_K, split_k_factor::1, stages::4
    std::vector<int>               occupancies(candidate_configs.size());

    // 选取stage值为2, 3, 4时的最佳运行配置，找到一个最能有效利用计算资源的配置
    // 用于评估
...
template<typename T, typename WeightType>
void MoeGemmRunner<T, WeightType>::moe_gemm_bias_act(const T*          A, // permuted_data_
                                                     const WeightType* B, // expert_weights
                                                     const T*          weight_scales,
                                                     const T*          biases,  // expert_biases
                                                     T*                C, // result
                                                     int64_t*          total_rows_before_expert,
                                                     int64_t           total_rows,
                                                     int64_t           gemm_n,  // d_ffn
                                                     int64_t           gemm_k,  // hidden_size=d_model
                                                     int               num_experts,
